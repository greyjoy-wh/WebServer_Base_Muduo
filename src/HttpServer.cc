...
HttpServer::HttpServer(EventLoop *loop,
    ...
    : server_(loop, listenAddr, name, option), user_(user), 
      passwd_(passwd), databaseName_(databaseName), sqlNum_(sqlNum)
{
...
void HttpServer::initmysql(ConnectionPool *connPool)
{
    ...
    {
        LOG_ERROR("SELECT error: %d", mysql_error(mysql));
    }
...
{
    LOG_INFO("HttpServer[ %c ] starts listening on %c", server_.name().c_str(), server_.ipPort().c_str());
    server_.start();
    // 初始化数据库连接池
    connPool_ = ConnectionPool::getInstance();
    connPool_->init("127.0.0.1", user_, passwd_, databaseName_, 3306, sqlNum_); //mysql 在哪个端口默认3306
    initmysql(connPool_);
}
