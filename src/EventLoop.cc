...
EventLoop::EventLoop()
    ...
    , threadId_(CurrentThread::tid()) //获取该线程的线程号
    ...
    , wakeupChannel_(new Channel(this, wakeupFd_))
{
    ...
    wakeupChannel_->enableReading(); //将wakeupfd加入到epoll的event事件中
}
...
{
    {
        //为什么需要锁这里，除了本线程还有其他线程来添加pendingfunctor吗
        //其他线程特别是main线程 需要往里面添加回调函数，本线程同时也会取出函数执行，这样就涉及并发了
...
void EventLoop::doPendingFunctors() // 执行回调
{
    ...
    {
        std::unique_lock<std::mutex> lock(mutex_);
        functors.swap(pendingFunctors_);//换到栈空间上来就可以快速丢去锁。
    }
